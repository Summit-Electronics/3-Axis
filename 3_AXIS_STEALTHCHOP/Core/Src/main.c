...
/**
  ...
  *
  ******************************************************************************
  * Example: 3_AXIS_STEALTHCHOP
  * Board: 3-AXIS Stepper Driver
  * Purpose: Demonstrates how the Stealthchop function can be used to reduce motor
  * Vibration and sound at low speeds.
  * Motor connected to axis 1 is set to spin at 0,5 RPS for 5 sec using normal
  * operation mode. After these 5 sec, Stealthchop is initiated and the motor
  * will keep rotatingfor 10 sec. User should experience reduced sound and
  * vibration with Stealthchop active.
  */
...
int main(void)
{
  ...
  Current_axis_1.IHOLD 	= 5;
  Current_axis_1.IRUN 	= 15;
  ...
  TMC5160_Basic_Init(&Current_axis_3, 3); //init axis 3
   ...
   TMC5160_Drive_Enable(1,1); //Enable Drive

   TMC5160_Basic_Rotate(1,&Ramp_axis_1, 1); // Constant rotation
   HAL_Delay(5000);
   TMC5160_Stop(1); // Stop movement

   TMC5160_Init_Stealthchop(1);
   TMC5160_Basic_Rotate(1,&Ramp_axis_1,1); // Constant rotation
   HAL_Delay(10000);

   TMC5160_Stop(1); // Stop movement

   TMC5160_Drive_Enable(0,1); //Disable Drive
