...
/**
  ...
  *
  ******************************************************************************
  * Example: 3_AXIS_FDCAN_Send
  * Board: 3-AXIS Stepper Driver
  * Purpose: Demonstrates communication using CAN bus.
  * This code will send an initial message with CANTxData[0] = 0x01
  * after it will listen to incoming CAN bus messages and once CANRxData[0] == 0x01
  * has been received, respond back.
  *
  * Note: FDCAN is not supported on 1-axis. if communication with 1-AXIS is needed
  * use regular CAN
  *
  * Baud used: 1Mbps
  */
...
int main(void)
{
  ...
  //HAL_NVIC_EnableIRQ(TIM16_FDCAN_IT0_IRQn); // test


    // DATA pakket opbouw CAN BUS
  CANTxHeader.Identifier = 0x200; //ID
  CANTxHeader.IdType = FDCAN_STANDARD_ID; // ID type
  CANTxHeader.TxFrameType = FDCAN_DATA_FRAME;
  CANTxHeader.DataLength = 8;	// data length
  CANTxHeader.BitRateSwitch = 1; //Bitrate switch to 5 Mbps on
