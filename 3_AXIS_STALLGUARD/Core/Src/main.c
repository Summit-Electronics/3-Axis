...
/**
  ...
  *
  ******************************************************************************
  * Example: 3_AXIS_STALLGUARD
  * Board: 3-AXIS Stepper Driver
  * Purpose: Demonstrates how the Stallguard function can be used to detect stalls
  * In this example it's set up to work as sensorless homing on axis 1.
  * Motor is set to spin at 1 RPS with very low current, to allow the user to safely
  * stall the motor. Once the motor stalls. it will set the current position to 0.
  */
...
const int STG_ENB_A1 = 1;  // 0 = disable Stallguard, 1 = enable Stallguard
...
int main(void)
{
  ...
  StallSettings.VSTART = 10;
  StallSettings.AMAX 	= 51200;
  StallSettings.VMAX 	= 51200;
  StallSettings.VSTOP 	= 10;
  ...
  Current_axis_1.IRUN 	= 2;

  Current_axis_2.IHOLD 	= 1;
  Current_axis_2.IRUN 	= 2;

  Current_axis_3.IHOLD 	= 1;
  Current_axis_3.IRUN 	= 2;
  ...
  TMC5160_Basic_Rotate(1, &StallSettings, 1);
  TMC5160_Init_Stallguard(0, 1, 1);//enable stallguard

  while (stall == 0) //wait for stepper to stall
  {
  	if(TMC5160_Monitor_Stallguard(1) == 1) //stall checker function
  	{
  		TMC5160_Drive_Enable(0, 1);
  		TMC5160_Init_Stallguard(1,0,1); // clear stall flag
  		stall = 1;
  	}
  }

  TMC5160_Set_Home(1); //set current position as home "0"
////////////////////////////////////////////////////////////////////////////////////

  // start moving from Home position.
  TMC5160_Basic_Rotate(1, &StallSettings, 1); //start basic movement
  TMC5160_Init_Stallguard(0,1,1); //enable stallguard
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	if (TMC5160_Monitor_Stallguard(1) == 1)
	{
		// stall event
		TMC5160_Drive_Enable(0,1);
		TMC5160_Init_Stallguard(1,0,1); // clear stall flag
		...
		TMC5160_Basic_Rotate(1, &StallSettings,1); // continue movement after stall (if needed)
	}
